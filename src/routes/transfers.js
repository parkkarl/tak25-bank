...
router.post('/transfers', authenticate, async (req, res) => {
  ...
  try {
    const rates = await getExchangeRates();
    // Lookup destination account currency
    ...
    if (lookupRes.ok) {
      ...
      if (src.currency !== destInfo.currency) {
        ...
        const converted = (parseFloat(amount) / srcRate) * dstRate;
...
router.post('/transfers/receive', async (req, res) => {
  ...
  try {
    // Decode without verifying to get sourceBankId
...
// Retry pending transfers (background worker)
  ...
  setInterval(async () => {
    ...
    for (const t of pending) {
      ...
      if (Date.now() - new Date(t.pending_since).getTime() > 4 * 60 * 60 * 1000) {
        ...
        if (src) {
          db.prepare('UPDATE accounts SET balance = ? WHERE account_number = ?')
            .run((parseFloat(src.balance) + parseFloat(t.amount)).toFixed(2), t.source_account);
        }
      ...
      try {
        const jwt = await signJwt({
          transferId: t.transfer_id, sourceAccount: t.source_account,
          destinationAccount: t.destination_account, amount: t.converted_amount || t.amount,
          sourceBankId: getBankId(), destinationBankId: destBank.bankId,
        ...
        if (tfRes.ok) {
          db.prepare(`UPDATE transfers SET status = 'completed' WHERE transfer_id = ?`).run(t.transfer_id);
        } else {
        ...
        db.prepare('UPDATE transfers SET retry_count = ?, next_retry_at = ? WHERE transfer_id = ?')
          .run(retryCount, nextRetryAt, t.transfer_id);
      }
    }
  }, 30000);
}
